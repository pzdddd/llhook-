...
object TrackHook : BaseHook {
        ...
        try {
            ...
            fragmentClass.findMethod { name == "onResume" }.hookAfter { param ->
                    ...
                    setOnClickListener {
                        ...
                        if (uid != null) {
                            ...
                            if (res != null) {
                                ...
                                startTracking(activity, uid, true) { r ->
                ...
                thread {
                    ...
                    // 自动模式: 进入主页即后台静默解算; 手动模式跳过, 等待用户点击「追踪」按钮
                    if (!isManualMode) {
                        startTracking(activity, uidFound, false, null)
                    }
                }
            }
    ...
    // =========================================================================
    //  统一解算入口 (自动 / 手动复用)
    //   - manual=false: 进入主页自动后台解算 (默认, 行为同原版)
    //   - manual=true : 用户点击「追踪」才解算; onResult 回调用于点击后即时展示结果
    // =========================================================================
    private fun startTracking(
        activity: Activity, uid: String, manual: Boolean, onResult: ((TrackResult) -> Unit)?
    ) {
        thread {
            val token = Config.getAuthToken(activity)
            if (token.isEmpty()) {
